...
#blocked out rules are due to hard drive limitations, if you have unlimited HD space, feel free to run the whole workflow in 1 run through

configfile: "Snakefile_config.json"
...
rule fastqc_analysis:	
	...
	shell:
		"""
		fastqc {input.fqc_in1} -o {params.raw_reads};
		fastqc {input.fqc_in2} -o {params.raw_reads}
		"""

rule trim_galore_pe:
	input:
		trim = [os.path.join(raw_reads + "{sample}.1.fq.gz"), os.path.join(raw_reads + "{sample}.2.fq.gz")],
	output:
...
rule minimap2_bam:
	...
	shell:
		"""
		minimap2 -ax sr -t6 {params.param} {input.target} {input.query1} {input.query2} | samtools fixmate -@ 6 - - | samtools sort -@ 6 -O bam - -o {output.bam};
...
rule freebayes:
	...
	output:
		...
		index = os.path.join(called_reads + "freebayes_calls.vcf.gz.tbi"),
	shell:
		"""
		freebayes-parallel <(fasta_generate_regions.py {input.target_fai} 10000) 42 -f {input.target} -g 200 --report-monomorphic --gvcf -L {input.bam_list} | bgzip -c > {output.vcf}
		tabix {output.vcf}
		"""
...
rule multiqc_all:
	input:
		expand(os.path.join(raw_fastqc + "{sample}_1_fastqc.html"), sample=sample),
		expand(os.path.join(trimmed_reads + "{sample}_1_val_1_fastqc.html"), sample=sample),
